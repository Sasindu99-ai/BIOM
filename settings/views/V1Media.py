...
class V1Media(API):
	...
	def uploadStreamFile(self, request):
		...
		if is_image_file:
			# For profile photos: profile_{timestamp}_{random}.ext
			file_name = f'profile_{timestamp}_{random_str}{file_ext}'
		else:
			# For bulk files: keep original name with timestamp
			file_name = f'{timestamp}_{uploaded_file.name}'
		
		file_path = upload_dir / file_name
		
		try:
			# Save file with streaming chunks
			...
			Logger.error(f'Error uploading streaming file: {str(e)}')
